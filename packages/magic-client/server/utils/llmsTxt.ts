...
const SUMMARY = [
  '> Interactive computer science theory. Build a graph on an infinite canvas and',
  '> watch an algorithm run over it one step at a time, alone or in a shared room.',
];

/**
 * the body of `/llms.txt`, kept pure so the one invariant that matters -- that it lists
 * the same pages the sitemap does -- can be tested without standing up nitro
 */
