...
const createFakeGraph = () => {
  ...
  const addElements = ({
    ...
    for (const edge of addedEdges) edges.push({ ...edge });
    rawEvents.emit('onElementsAdded', {
      addedNodes: addedNodes.map(({ id }) => ({ id })),
      addedEdges: addedEdges.map(({ id }) => ({ id })),
    } as never);
  };
  ...
  const graph = {
    ...
    getNode,
    getEdge,
    isNode: (id: string) => getNode(id) !== undefined,
    positions,
...
describe(bindGraphToDoc, () => {
  ...
  it('applies a remote add, relabel, reweight and removal', () => {
    ...
    expect(peer.graph.getEdge('a-c')).toBeUndefined();
  });
